...
test("uses Japanese browser fallback without persisting inferred locale", async () => {
  ...
  try {
    ...
    assert.equal(rendered.document.querySelector('button[type="button"]')?.textContent, "English");
    assert.equal(rendered.window.localStorage.getItem("narrativeline.language"), null);
...
test("falls back to English for unsupported browser locales without persisting", async () => {
  ...
  try {
    ...
    assert.notEqual(rendered.document.querySelector('button[type="button"]')?.textContent, "English");
    assert.equal(rendered.window.localStorage.getItem("narrativeline.language"), null);
  } finally {
    rendered.cleanup();
  }
