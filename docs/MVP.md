...
NarrativeLine provides interpretation and one bounded writer workflow for the
unregistered Coordinate interoperability format `0.1.0`.
...
- Event Coordinates and non-LiaisonScape Spaces remain read-only.
- Entity Detail may explicitly edit existing `x` and `y` values only when the
  selected Space has the exact Cartesian kind, units, and positive directions
  supported by both applications. The canonical current Space identifier is
  `liaisonscape-graph`; legacy `linkscape-graph` data remains supported for
  compatibility.
...
This support does not make Coordinate a registered Stable Extension and does
not turn NarrativeLine into a spatial or graph editor.
